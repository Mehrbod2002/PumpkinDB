...
fn main() {
    ...
    let settings = config::Config::builder()
        ...
        .set_default("prompt", "PumpkinDB>").unwrap().build().unwrap();
    let formatted_prompt = format!("{}", settings.get_string("prompt").unwrap());
            ...
            Ok(text) => {
                ...
                if text_bytes.len() >= 2 && text_bytes[0] == b'\\' {
                    if text_bytes[1] == b'h' {
                        ...
                        println!(
                            "Missing a feature? Let us know at \
                                    ...
                                    .receive(|r: &[u8]| {
                                        if r[0..5].to_vec() == b"TRACE" {
                                            ...
                                            if cfg!(target_os = "windows") {
                                                ...
                                                let _ = write!(&mut s, "{}", Cyan.paint("Trace: "));
                                            }
                                            match pumpkinscript::binparser::data(&input.clone()) {
                                                pumpkinscript::ParseResult::Done(_, data) => {
                                                    let (_, size) =
                                                        pumpkinscript::binparser::data_size(data)
                                                            .unwrap();
                                                    let data =
                                                        &data[script::offset_by_size(size)..];
                                            ...
                                            while input.len() > 0 {
                                                match pumpkinscript::binparser::data(&input.clone())
                                                {
                                                    pumpkinscript::ParseResult::Done(
                                                        ...
                                                        let (_, size) =
                                                            pumpkinscript::binparser::data_size(
                                                                data,
                                                            )
                                                            .unwrap();
                                                        let data =
